...
/** vscode-free doc-type defaults, kept out of `docTypes.ts` (which imports `vscode` for
 * `getDocTypeDefinitions`'s workspace-settings lookup) so the standalone MCP server (`crawlNode.ts`) can use
 * the built-in defaults without transitively requiring `vscode`. */
...
export const DEFAULT_DOC_TYPES: DocTypeDefinition[] = [
...
export function cloneDocTypes(defs: DocTypeDefinition[]): DocTypeDefinition[] {
